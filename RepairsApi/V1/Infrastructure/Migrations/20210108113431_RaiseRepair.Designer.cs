// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using RepairsApi.V1.Infrastructure;

namespace RepairsApi.V1.Infrastructure.Migrations
{
    [DbContext(typeof(RepairsContext))]
    [Migration("20210108113431_RaiseRepair")]
    partial class RaiseRepair
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn)
                .HasAnnotation("ProductVersion", "3.1.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.LocationAlert", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Comments")
                        .HasColumnName("comments")
                        .HasColumnType("text");

                    b.Property<int>("Type")
                        .HasColumnName("type")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("LocationAlerts");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.PersonAlert", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Comments")
                        .HasColumnName("comments")
                        .HasColumnType("text");

                    b.Property<int>("Type")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("PersonAlerts");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.RateScheduleItem", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("uuid");

                    b.Property<string>("CustomCode")
                        .HasColumnName("custom_code")
                        .HasColumnType("text");

                    b.Property<string>("CustomName")
                        .HasColumnName("custom_name")
                        .HasColumnType("text");

                    b.Property<Guid>("WorkElementId")
                        .HasColumnName("work_element_id")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("WorkElementId");

                    b.ToTable("rate_schedule_item");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.SitePropertyUnit", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Reference")
                        .HasColumnName("reference")
                        .HasColumnType("text");

                    b.Property<int?>("WorkOrderId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("WorkOrderId");

                    b.ToTable("SitePropertyUnit");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkElement", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("uuid");

                    b.Property<bool>("ContainsCapitalWork")
                        .HasColumnName("contains_capital_work")
                        .HasColumnType("boolean");

                    b.Property<string>("ServiceChargeSubject")
                        .HasColumnName("service_charge_subject")
                        .HasColumnType("text");

                    b.Property<string>("Trade")
                        .HasColumnName("trade ")
                        .HasColumnType("text");

                    b.Property<int?>("WorkOrderId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("WorkOrderId");

                    b.ToTable("work_element");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkOrder", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:IdentitySequenceOptions", "'10000000', '1', '', '', 'False', '1'")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("DescriptionOfWork")
                        .HasColumnName("description_of_work")
                        .HasColumnType("text");

                    b.Property<Guid?>("PriorityId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("PriorityId");

                    b.ToTable("work_order");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkPriority", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("uuid");

                    b.Property<string>("Comments")
                        .HasColumnName("comments")
                        .HasColumnType("text");

                    b.Property<double>("NumberOfDays")
                        .HasColumnName("number_of_days")
                        .HasColumnType("double precision");

                    b.Property<int?>("PriorityCodeId")
                        .HasColumnType("integer");

                    b.Property<string>("PriorityDescription")
                        .HasColumnName("priority_description")
                        .HasColumnType("text");

                    b.Property<DateTime>("RequiredCompletionDateTime")
                        .HasColumnName("required_completion_datetime")
                        .HasColumnType("timestamp without time zone");

                    b.HasKey("Id");

                    b.HasIndex("PriorityCodeId");

                    b.ToTable("work_priority");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkPriorityCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id")
                        .HasColumnType("integer")
                        .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn);

                    b.Property<string>("Name")
                        .HasColumnName("name")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("work_priority_code");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Emergency"
                        },
                        new
                        {
                            Id = 2,
                            Name = "High"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Medium"
                        },
                        new
                        {
                            Id = 4,
                            Name = "Low"
                        },
                        new
                        {
                            Id = 5,
                            Name = "Deferred"
                        });
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.RateScheduleItem", b =>
                {
                    b.HasOne("RepairsApi.V1.Infrastructure.WorkElement", "WorkElement")
                        .WithMany("RateScheduleItem")
                        .HasForeignKey("WorkElementId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("RepairsApi.V1.Infrastructure.Quantity", "Quantity", b1 =>
                        {
                            b1.Property<Guid>("RateScheduleItemId")
                                .HasColumnType("uuid");

                            b1.Property<int>("Amount")
                                .HasColumnName("amount")
                                .HasColumnType("integer");

                            b1.Property<string>("UnitOfMeasurementCode")
                                .HasColumnName("unit_of_measurement_code")
                                .HasColumnType("text");

                            b1.HasKey("RateScheduleItemId");

                            b1.ToTable("rate_schedule_item");

                            b1.WithOwner()
                                .HasForeignKey("RateScheduleItemId");
                        });
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.SitePropertyUnit", b =>
                {
                    b.HasOne("RepairsApi.V1.Infrastructure.WorkOrder", null)
                        .WithMany("SitePropertyUnits")
                        .HasForeignKey("WorkOrderId");

                    b.OwnsOne("RepairsApi.V1.Infrastructure.Address", "Address", b1 =>
                        {
                            b1.Property<int>("SitePropertyUnitId")
                                .HasColumnType("integer");

                            b1.Property<string>("AddressLine")
                                .HasColumnName("address_line")
                                .HasColumnType("text");

                            b1.Property<string>("BuildingName")
                                .HasColumnName("address_building_name")
                                .HasColumnType("text");

                            b1.Property<string>("BuildingNumber")
                                .HasColumnName("address_building_number")
                                .HasColumnType("text");

                            b1.Property<string>("CityName")
                                .HasColumnName("address_city_name")
                                .HasColumnType("text");

                            b1.Property<string>("ComplexName")
                                .HasColumnName("address_complex_name")
                                .HasColumnType("text");

                            b1.Property<int>("Country")
                                .HasColumnName("address_country")
                                .HasColumnType("integer");

                            b1.Property<string>("Department")
                                .HasColumnName("address_department")
                                .HasColumnType("text");

                            b1.Property<string>("Floor")
                                .HasColumnName("address_floor")
                                .HasColumnType("text");

                            b1.Property<string>("Plot")
                                .HasColumnName("address_plot")
                                .HasColumnType("text");

                            b1.Property<string>("PostalCode")
                                .HasColumnName("address_postal_code")
                                .HasColumnType("text");

                            b1.Property<string>("Postbox")
                                .HasColumnName("address_postbox")
                                .HasColumnType("text");

                            b1.Property<string>("Room")
                                .HasColumnName("address_room")
                                .HasColumnType("text");

                            b1.Property<string>("StreetName")
                                .HasColumnName("address_street_name")
                                .HasColumnType("text");

                            b1.Property<string>("Type")
                                .HasColumnName("address_type")
                                .HasColumnType("text");

                            b1.HasKey("SitePropertyUnitId");

                            b1.ToTable("SitePropertyUnit");

                            b1.WithOwner()
                                .HasForeignKey("SitePropertyUnitId");
                        });
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkElement", b =>
                {
                    b.HasOne("RepairsApi.V1.Infrastructure.WorkOrder", null)
                        .WithMany("WorkElements")
                        .HasForeignKey("WorkOrderId");
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkOrder", b =>
                {
                    b.HasOne("RepairsApi.V1.Infrastructure.WorkPriority", "Priority")
                        .WithMany()
                        .HasForeignKey("PriorityId");

                    b.OwnsOne("RepairsApi.V1.Infrastructure.WorkClass", "WorkClass", b1 =>
                        {
                            b1.Property<int>("WorkOrderId")
                                .HasColumnType("integer");

                            b1.Property<int>("WorkClassCode")
                                .HasColumnName("work_class_code")
                                .HasColumnType("integer");

                            b1.HasKey("WorkOrderId");

                            b1.ToTable("work_order");

                            b1.WithOwner()
                                .HasForeignKey("WorkOrderId");
                        });
                });

            modelBuilder.Entity("RepairsApi.V1.Infrastructure.WorkPriority", b =>
                {
                    b.HasOne("RepairsApi.V1.Infrastructure.WorkPriorityCode", "PriorityCode")
                        .WithMany()
                        .HasForeignKey("PriorityCodeId");
                });
#pragma warning restore 612, 618
        }
    }
}
